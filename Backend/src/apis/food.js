...
const getFoodByMenuId = router.get('/food', async (req, res, next) => {
...
const getFoodById = router.get('/foodById', async (req, res, next) => {
  console.log(req.query);
  if (!_.has(req, 'query.key') || req.query.key !== API_KEY) {
    res.send(JSON.stringify({ success: false, message: 'Wrong API key' }));
  } else {
...
const searchFoodByName = router.get('/foodSearchByName', async (req, res, next) => {
  console.log(req.query);
  if (!_.has(req, 'query.key') || req.query.key !== API_KEY) {
    res.send(JSON.stringify({ success: false, message: 'Wrong API key' }));
  } else {
